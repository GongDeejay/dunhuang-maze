...
func _rebuild_hit_zones() -> void:
	...
	var alpha := 0.68
	var density := clampf(rect.size.x / 390.0, 1.0, 2.0)
	var fs := int(clampf(rect.size.y * 0.12 * layout.ui_scale, 22.0 * density, 34.0 * density))
	if layout.is_portrait():
...
func _draw_portrait_controls(rect: Rect2, alpha: float, fs: int) -> void:
	...
	for entry in _dpad_centers:
		...
		draw_circle(pos, btn_radius, Color(1, 1, 1, alpha * 0.42), false, 2.0)
		draw_string(ThemeDB.fallback_font, pos + Vector2(-btn_radius, fs * 0.34), label,
			HORIZONTAL_ALIGNMENT_CENTER, int(btn_radius * 2.0), fs, Color(1, 1, 1, alpha))
	...
	for action in _func_centers:
		...
		draw_rect(btn_rect, Color(1, 1, 1, alpha * 0.24))
		draw_rect(btn_rect, Color(1, 1, 1, alpha * 0.42), false, 2.0)
		draw_string(ThemeDB.fallback_font, Vector2(btn_rect.position.x, pos.y + fs * 0.26), label,
			HORIZONTAL_ALIGNMENT_CENTER, int(func_btn_size), int(fs * 0.72), Color(1, 1, 1, alpha))


func _layout_portrait_zones(rect: Rect2, _alpha: float, _fs: int) -> void:
	var dpad_cx := rect.position.x + rect.size.x * 0.25
	...
	var cols := 3
	var start_x := rect.position.x + rect.size.x * 0.575
	var start_y := rect.position.y + rect.size.y * 0.32
	var gap_x := minf(func_btn_size + 8.0, rect.size.x * 0.17)
