...
发布提交 `295760c`，Build and Deploy Web 运行 `35187520387` 成功，Test 运行 `35187520365` 成功。212 项游戏断言、3 项 Python 发布缓存测试及 5 组浏览器 UI 检查通过。Godot 测试退出时仍有已有的 RID/资源泄漏告警，未将其计作新增问题已修复。

生产浏览器实测（含约 0.7 秒首帧截图等待）：

- 全新无痕测试环境两次冷启动约 11.6 秒、52.2 秒；较大 WASM 在该无痕环境未命中回访缓存。
- 使用全新独立持久化浏览器配置，冷启动约 100.2 秒，其中 WASM 下载约 98.1 秒。
- 同一普通浏览器配置第二次进入约 1.6 秒；WASM 与 PCK 的 Resource Timing `transferSize` 均为 **0**，确认无需重新下载。
- 以上不是统一网络条件下的前后性能对照，不能宣称首次加载稳定在某个秒数。网络链路瓶颈未完全解决；无痕、清缓存、强制刷新也不保证复用资源。
