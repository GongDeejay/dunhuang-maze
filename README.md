...
引擎：`gl_compatibility`，布局直接适配浏览器尺寸，Safari 兼容（无线程）。
...
|------|------|
| WASD | 移动 |
| P / Esc | 暂停 / 继续（切出浏览器时自动暂停） |
| 1–5 | 选择背包槽 |
| E | 使用道具 |
| R | 重开本关 / 胜利后下一关 |
| Q | 返回难度选择 |

朝相邻敌人的方向移动即可攻击（攻击间隔 0.35 秒），不会与敌人重叠。敌人按实时节奏独立行动，攻击前显示下一步意图，可后退躲避。移动端使用屏幕下方的方向、道具、暂停、重开和菜单按钮。
